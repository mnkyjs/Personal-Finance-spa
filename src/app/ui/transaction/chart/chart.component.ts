...
export class ChartComponent implements OnInit {
    ...
    this.barChartData = [
      {
        ...
        label: this.transactionType === TransactionTypeEnum.Income ? 'Einnahmen' : 'Ausgaben',
        backgroundColor: this.transactionType === TransactionTypeEnum.Income ? '#66BB6A' : '#ef5350'
      }
    ]
  }
  ...
  private _prepareTransactionArray(): void {
    switch (this.transactionType) {
      case TransactionTypeEnum.Income:
        this.transactionForChartView = [...this.transactions?.filter(t => t.transactionType === TransactionTypeEnum.Income)];
        break;

      case TransactionTypeEnum.Expense:
        this.transactionForChartView = [...this.transactions?.filter(t => t.transactionType === TransactionTypeEnum.Expense)];
        break;
  ...
  private _prepareDataForChart(): void {
    ...
    let currentAmount = 0;
    let sortedArray = [...this.transactionForChartView.sort((a, b) => (a.categoryId > b.categoryId) ? 1 : -1)]
    ...
    for (const transaction of sortedArray) {
      if (transaction.categoryId === currentCategoryId) {
        tempCategories.indexOf(this.categories.find(c => c.id === transaction.categoryId).name) === -1 && tempCategories.push(this.categories.find(c => c.id === transaction.categoryId).name);
