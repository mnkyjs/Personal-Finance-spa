...
import { CategorieDto, TransactionDto, TransactionTypeEnum } from '../../../api/service/personal-finance-api.service';
                ...
                if (transaction.transactionType === TransactionTypeEnum.Einnahmen) {
                    if (this.barChartData[0].data.length > 0) {
                        this.barChartData[0].data.push(
                            (this.barChartData[0]?.data.slice(-1)[0] as number) + transaction.value
                        );
                    } else {
                        this.barChartData[0].data.push(transaction.value);
                    }
                    this.barChartLabels.push(transaction.date.toLocaleString('de-De').slice(0, 9));
                }
                ...
                if (transaction.transactionType === TransactionTypeEnum.Ausgaben) {
                    if (this.barChartData[1].data.length > 0) {
                        this.barChartData[1].data.push(
                            (this.barChartData[1]?.data.slice(-1)[0] as number) + transaction.value * -1
                        );
                    } else {
